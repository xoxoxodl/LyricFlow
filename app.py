import streamlit as st
import os
import spotipy
import lyricsgenius
import anthropic
from spotipy.oauth2 import SpotifyOAuth
from dotenv import load_dotenv

# 1. 환경 설정 및 세션 관리
load_dotenv()

# .env에서 환경 변수 안전하게 로드
SPOTIPY_CLIENT_ID = os.getenv("SPOTIPY_CLIENT_ID")
SPOTIPY_CLIENT_SECRET = os.getenv("SPOTIPY_CLIENT_SECRET")
SPOTIPY_REDIRECT_URI = os.getenv("SPOTIPY_REDIRECT_URI")
GENIUS_ACCESS_TOKEN = os.getenv("GENIUS_ACCESS_TOKEN")
ANTHROPIC_API_KEY = os.getenv("ANTHROPIC_API_KEY")

# Streamlit 페이지 설정
st.set_page_config(page_title="LyricFlow", page_icon="🎵", layout="centered")

# 디자인 커스텀 (스포티파이 다크 테마)
st.markdown("""
    <style>
    .stApp { background-color: #121212; color: white; }
    .stButton>button { 
        background-color: #1DB954; color: white; border-radius: 25px; 
        width: 100%; border: none; font-weight: bold; padding: 12px;
        transition: 0.3s;
    }
    .stButton>button:hover { background-color: #1ed760; transform: scale(1.02); }
    .stInfo { background-color: #282828; color: #ffffff; border: none; border-radius: 12px; }
    .stSuccess { background-color: #1DB954; color: white; border: none; border-radius: 12px; }
    </style>
    """, unsafe_allow_html=True)

# 2. API 인증 객체 생성
sp = spotipy.Spotify(auth_manager=SpotifyOAuth(
    client_id=SPOTIPY_CLIENT_ID,
    client_secret=SPOTIPY_CLIENT_SECRET,
    redirect_uri=SPOTIPY_REDIRECT_URI,
    scope="user-read-currently-playing"
))

genius = lyricsgenius.Genius(GENIUS_ACCESS_TOKEN)
genius.remove_section_headers = True

# 3. 메인 UI 구성
st.title("🎵 LyricFlow")
st.write("지금 듣는 노래의 가사를 분석하고, 무드가 이어지는 다음 곡을 추천합니다.")
st.divider()

if st.button('지금 재생 중인 노래 분석하기'):
    with st.spinner('음악 정보를 가져오는 중...'):
        current_playing = sp.current_user_playing_track()
        
        if current_playing and current_playing.get('item'):
            track = current_playing['item']
            track_name = track['name']
            artists = ", ".join([artist['name'] for artist in track['artists']])
            album_img = track['album']['images'][0]['url']
            
            # 노래 정보 출력
            col_img, col_info = st.columns([1, 2])
            with col_img:
                st.image(album_img, use_container_width=True)
            with col_info:
                st.header(track_name)
                st.subheader(artists)

            # 가사 검색 및 Claude 분석
            with st.spinner('Claude가 가사의 서사를 분석 중입니다...'):
                song = genius.search_song(track_name, artists)
                
                if song:
                    client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)
                    
                    prompt = f"""당신은 음악 가사 분석 전문가입니다. 아래 노래의 가사를 분석해 주세요.

곡 제목: {track_name}
아티스트: {artists}
가사 전문: {song.lyrics}

지시사항:
- 요약: 가사의 주제와 감정선을 3줄로 요약해 주세요. (제목이나 번호 생략)
- 키워드: 이 곡의 분위기를 나타내는 키워드 3~5개를 콤마로 구분해서 뽑아주세요.
- 추천: 이 곡과 서사가 이어지는 추천곡 '3곡'을 제안해 주세요.

출력 형식 (반드시 지킬 것):
요약: [내용]
키워드: [키워드1, 키워드2, 키워드3]
추천1: [곡 제목 - 아티스트] | [이유]
추천2: [곡 제목 - 아티스트] | [이유]
추천3: [곡 제목 - 아티스트] | [이유]"""
                    
                    # 테이님이 성공하신 모델명 적용
                    message = client.messages.create(
                        model="claude-sonnet-4-6", 
                        max_tokens=1500,
                        temperature=0.7,
                        messages=[{"role": "user", "content": prompt}]
                    )
                    
                    full_response = message.content[0].text
                    
                    # --- 결과 표시 시작 ---
                    st.divider()
                    st.markdown("### 📝 가사 감성 리포트")
                    
                    # 데이터 파싱
                    summary_part = full_response.split("키워드:")[0].replace("요약:", "").strip()
                    keywords_raw = full_response.split("키워드:")[1].split("추천1:")[0].strip()
                    keywords = [k.strip() for k in keywords_raw.split(",")]
                    
                    # 1. 요약문 출력
                    st.info(summary_part)
                    
                    # 2. 키워드 배지 출력 (HTML/CSS 사용)
                    kw_html = ""
                    for k in keywords:
                        kw_html += f'<span style="background-color: #333; color: #1DB954; padding: 6px 14px; border-radius: 20px; margin-right: 8px; font-size: 0.85rem; font-weight: bold; border: 1px solid #1DB954;">#{k}</span>'
                    st.markdown(kw_html, unsafe_allow_html=True)
                    st.write("") # 간격용

                    # 3. 추천곡 섹션 (필터링 로직 포함)
                    st.divider()
                    st.markdown("### 💿 다음 Flow 추천곡")
                    
                    recommendations_raw = []
                    for i in range(1, 4):
                        marker = f"추천{i}:"
                        next_marker = f"추천{i+1}:" if i < 3 else "EOF"
                        if marker in full_response:
                            part = full_response.split(marker)[1].split(next_marker)[0].strip()
                            if "|" in part:
                                title_artist, reason = part.split("|")
                                recommendations_raw.append({"query": title_artist.strip(), "reason": reason.strip()})

                    # 스포티파이에서 실제 존재하는 곡만 필터링
                    valid_recs = []
                    for rec in recommendations_raw:
                        search_res = sp.search(q=rec['query'], type='track', limit=1)
                        if search_res['tracks']['items']:
                            r_track = search_res['tracks']['items'][0]
                            valid_recs.append({
                                "url": r_track['external_urls']['spotify'],
                                "img": r_track['album']['images'][0]['url'],
                                "name": r_track['name'],
                                "artist": r_track['artists'][0]['name'],
                                "reason": rec['reason']
                            })

                    # 필터링된 곡들만 화면에 출력
                    if valid_recs:
                        cols = st.columns(len(valid_recs))
                        for idx, rec in enumerate(valid_recs):
                            with cols[idx]:
                                st.image(rec['img'], use_container_width=True)
                                st.success(f"**{rec['name']}**\n\n{rec['artist']}")
                                st.caption(rec['reason'])
                                st.link_button("🎧 들어보기", rec['url'])
                    else:
                        st.warning("추천된 곡들을 스포티파이에서 찾을 수 없습니다.")
                else:
                    st.error("가사를 찾을 수 없습니다.")
        else:
            st.warning("현재 재생 중인 곡이 없습니다. 스포티파이를 확인해 주세요!")

st.caption(f"LyricFlow v1.1 | Developed by Tay Kim | Model: claude-sonnet-4-6")

